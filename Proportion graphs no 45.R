...
positive$Activity <- factor(c(rep(c("Games"), times = 8),rep(c("Music"), times = 8),rep(c("Sports"), times = 8)))


oo01 <- ggplot(data = positive, aes(x = Time, y = Proportion, group = Activity)) +
  geom_point() +
  scale_x_discrete(limits=c("10","20","30","40","50","60","70","80")) +
...
negative$Activity <- factor(c(rep(c("Traffic Jam"), times = 8),rep(c("Vacuum"), times = 8),rep(c("Dishes"), times = 8)))

oo02 <- ggplot(data = negative, aes(x = Time, y = Proportion, group = Activity)) +
  geom_point() +
  scale_x_discrete(limits=c("10","20","30","40","50","60","70","80")) +
  geom_line(aes(linetype = Activity)) +
  theme(plot.title = element_text(hjust=0.5)) +
